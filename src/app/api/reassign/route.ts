...
const executeCommands = async (
  ...
  const command = `graphman reassign ${subgraphId} ${newNode}`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { subgraphId, newNode } = await req.json();
    ...
    const result = await executeCommands(namespace, podName, containerName, subgraphId, newNode);
