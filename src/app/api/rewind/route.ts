...
const executeCommands = async (
  ...
  const command = `graphman rewind ${subgraphId} --block-hash ${blockHash} --block-number ${blockNumber}`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { subgraphId, blockHash, blockNumber } = await req.json();
    ...
    const result = await executeCommands(namespace, podName, containerName, subgraphId, blockHash, blockNumber);
